//! TMTO evaluation for v3 graphs (checkpoint replay under reduced resident set).

use super::config::{ComputeMemoryV3Config, TMTO_FRACTIONS};
...
impl<'a> CheckpointStore<'a> {
    fn new(cfg: &'a ComputeMemoryV3Config, seed: [u8; 32], stride: usize) -> Self {
    ...
    fn recompute_through(&mut self, idx: usize) {
        ...
        for j in base..=idx {
            let parents =
                graph::parents_for_node(self.cfg.graph, &state, j, self.cfg.fan_in, epoch_len);
            let parent_blocks = if j == 0 {
    ...
    fn run(&mut self) -> ([u64; 4], Vec<u8>) {
        ...
        for i in 0..num_blocks {
            let parents =
                graph::parents_for_node(self.cfg.graph, &state, i, self.cfg.fan_in, epoch_len);
...
impl TmtoEvaluator {
    pub fn evaluate(engine: &V3Engine, cfg: &ComputeMemoryV3Config) -> Vec<TmtoRecord> {
        let password = b"tmto_password_v3";
        let salt = b"tmto_salt_16b_v3!";
