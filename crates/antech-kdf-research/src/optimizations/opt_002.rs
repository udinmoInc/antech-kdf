...
//! Replaces block hashing overhead with u64 ARX updates to reduce CPU cycles while preserving memory traffic.

use super::{Candidate004Variant, OptParams};
use sha2::{Digest, Sha256};

pub struct Candidate004Opt002;

impl Candidate004Variant for Candidate004Opt002 {
    fn variant_id(&self) -> &'static str {
        "candidate-004-opt-002"
    }

    fn description(&self) -> &'static str {
        "u64 vectorized ARX churn (reduces CPU cycle overhead while maintaining memory bus traffic)"
    }
    ...
    fn derive(
        ...
        // Fast ARX churn step: state[0] = (state[0] + block_val).rotate_left(19) ^ step
