//! Screen asymmetry-focused CombinedFrontier changes vs far2+global baseline.
//! Goal: cut MT attacker more than single-verify cost. No artificial delay.
...
enum Variant {
    /// Current shipped: global + always-2 far from same post-local state.
    Far2Global,
    /// Mix global+far1, then compute far2 from *post-far1* state (3-phase).
    FarChain,
    /// Map both fars into older half of remote span.
    FarOldHalf,
    /// Far addresses bind all four state words more tightly.
    FarXorAll,
    /// FarChain + older-half second far.
    ChainOldHalf,
    /// Two distinct global gathers + always-2 far (same post-local state).
    Global2Far2,
    /// FarChain with xorall addressing on both phases.
    ChainXorAll,
    /// Serialize each remote parent as its own mix (no batched mix_views).
    RemoteSerialMix,
}
...
impl Variant {
    fn name(self) -> &'static str {
        match self {
            ...
            Variant::FarOldHalf => "far_oldhalf",
            Variant::FarXorAll => "far_xorall",
            Variant::ChainOldHalf => "chain_oldhalf",
            Variant::Global2Far2 => "global2_far2",
            Variant::ChainXorAll => "chain_xorall",
            Variant::RemoteSerialMix => "remote_serial",
        }
    }
...
#[inline(always)]
fn mix_serial(state: &mut [u64; 4], views: &[[u64; 4]; 8], n: usize) {
    for k in 0..n {
        mix_pair_words(state, &views[k], &views[k]);
    }
}
...
fn far1_addr(v: Variant, state: &[u64; 4], remote_span: usize) -> usize {
    let raw = match v {
        Variant::FarXorAll | Variant::ChainXorAll => {
            (state[0] ^ state[1].rotate_left(11) ^ state[2].wrapping_mul(C1) ^ state[3]) as usize
        }
        _ => ((state[1] ^ state[3].rotate_left(11)) as usize),
    } % remote_span;
    match v {
        Variant::FarOldHalf | Variant::ChainOldHalf => map_oldhalf(raw, remote_span),
        _ => raw,
    }
}
...
fn far2_addr(v: Variant, state: &[u64; 4], remote_span: usize) -> usize {
    let raw = match v {
        ...
        _ => ((state[0] ^ GOLDEN) as usize) % remote_span,
    };
    match v {
        Variant::FarOldHalf | Variant::ChainOldHalf => map_oldhalf(raw, remote_span),
        _ => raw,
...
/// Phase-A remotes: global (+ optional 2nd global) + far1. Used by all variants;
/// chained variants omit far2 here.
#[inline(always)]
fn parents_remote_a(v: Variant, state: &[u64; 4], i: u32) -> ([u32; 8], usize) {
    let mut indices = [0u32; 8];
    let mut len = 0usize;
    if i == 0 {
        return (indices, 0);
    ...
    if matches!(v, Variant::Global2Far2) && i_us > 1 {
        push_unique(
            &mut indices,
            &mut len,
    ...
    if i_us > fw + 1 {
        ...
        let far = far1_addr(v, state, remote_span);
        push_unique(&mut indices, &mut len, far as u32, i);
        let chained = matches!(
            v,
            Variant::FarChain | Variant::ChainOldHalf | Variant::ChainXorAll
        );
        if !chained {
            let far2 = far2_addr(v, state, remote_span);
...
/// Phase-B (chained only): far2 from post-A state.
#[inline(always)]
fn parents_remote_b(v: Variant, state: &[u64; 4], i: u32) -> ([u32; 8], usize) {
    let mut indices = [0u32; 8];
    let mut len = 0usize;
    let chained = matches!(
        v,
...
fn apply_scatter(buf: &mut [[u64; 4]], state: &[u64; 4], i: u32) {
...
fn derive_variant(
    ...
    let serial = matches!(v, Variant::RemoteSerialMix);
    ...
    for i in 0..N as u32 {
        if i == 0 {
            ...
            let (ri, rn) = parents_remote_a(v, &state, i);
            for k in 0..rn {
            ...
            if serial {
                mix_serial(&mut state, &views, rn);
            } else {
                mix_views(&mut state, &views, rn);
            }

            let (ri2, rn2) = parents_remote_b(v, &state, i);
            if rn2 > 0 {
                for k in 0..rn2 {
                    prefetch_block(buf, ri2[k]);
                }
                for k in 0..rn2 {
                    views[k] = buf[ri2[k] as usize];
                }
                if serial {
                    mix_serial(&mut state, &views, rn2);
                } else {
                    mix_views(&mut state, &views, rn2);
                }
            }
        }
        buf[i as usize] = state;
        apply_scatter(buf, &state, i);
    }
...
fn main() {
    ...
    println!("=== production defender (current shipped) ===");
    ...
    println!("=== asymm variant screen ===");
    println!("variant,def_p50,def_p95,att_1t,att_2t,att_4t,att_8t,att_16t,att_32t,cyc_16t,eff_16t");
    ...
    for &v in Variant::all() {
        ...
        let thread_set = [1usize, 2, 4, 8, 16, 32];
