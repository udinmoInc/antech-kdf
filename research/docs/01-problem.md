...
Production today is the **combined-frontier** compute-memory engine in `antech-kdf-core` (construction version 5, `$antech$v2$`). Early candidate forks and superseded campaigns are archived under `research/archive/` and are not the numbers to cite for the shipping library.
